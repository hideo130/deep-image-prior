...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    if netG == 'resnet_9blocks':
        ...
        net = MultiUnetGenerator(
            input_nc, output_nc, 8, ngf, norm_layer=norm_layer, use_dropout=use_dropout)
    elif netG == "unet_dec":
        from .network2 import DecreaseMiddleModel
        net = DecreaseMiddleModel(
            input_nc, output_nc, 8, ngf, norm_layer=norm_layer, use_dropout=use_dropout)
    else:
        raise NotImplementedError(
...
class MultiscaleDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d,
        ...
        """
        getIntermFeat Trueなら中間層の特徴量を近づけるlossを追加する．
        n_layers getIntermFeatで使うレイヤー数
...
class MultiUnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False):
        ...
        downs = []
        downs.append(UNetDown(input_nc, ngf, norm_layer))
        for i in [2, 4, 8]:
            downs.append(UNetDown(ngf * int(i / 2), ngf * i, norm_layer))
        for i in range(num_downs - 4):
            downs.append(UNetDown(ngf * 8, ngf * 8, norm_layer))

        self.common_model = nn.ModuleList(downs)

        # print(self.common_model)
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
            use_bias = norm_layer == nn.InstanceNorm2d
        ...
        for i in range(num_downs - 5):
            ups.append(UNetUp(ngf * 8, ngf * 8, norm_layer,
                              use_dropout=use_dropout))
        for i in [8, 4, 2]:
            ups.append(UNetUp(ngf * i, ngf * int(i / 2), norm_layer))
        ups.append(UNetUp(ngf, output_nc, norm_layer, is_output_layer=True))
        # ups.append(nn.Tanh())

        # self.up1 = nn.Sequential(*ups)
        # self.up2 = nn.Sequential(*ups)
        import copy
        ups2 = copy.deepcopy(ups)
        self.up1 = nn.ModuleList(ups)
        self.up2 = nn.ModuleList(ups2)
        # print(self.up1)
        self.final_activate1 = nn.Sequential(nn.Tanh())
        self.final_activate2 = nn.Sequential(nn.Tanh())

    def make_uplayers(self):
        # 作る必要がでてきたらUnetのアップサンプリング層を追加する．
        pass
    ...
    def forward(self, input):
        ...
        res = [input]
        # x = self.common_model(input)
        # print(self.common_model)
        for i in range(len(self.common_model)):
            layer_output = self.common_model[i](res[-1])
            res.append(layer_output)

        x1 = self.upconv(res[-1])
        x2 = self.upconv(res[-1])
        # for i in range(len(res)):
        #     print(i,":",res[i].size())

        for i in range(len(self.up1)):
            # print("UP NOW count=",i)
            # print("from former layer:",x1.size())
            # print("concat", res[len(self.up1)-i].size())
            """
            res[len(res)-1]は最下層
            res[len(res)-2]が最下層の前のlayer
            *[]
            """
            x1 = self.up1[i](x1, res[len(res) - 2 - i])
            x2 = self.up2[i](x2, res[len(res) - 2 - i])

        # print(x1.size())
        image1 = self.final_activate1(x1)
        image2 = self.final_activate2(x2)
        return image1, image2


class UNetDown(nn.Module):

    def __init__(self, in_size, out_size, norm_layer):
        super(UNetDown, self).__init__()
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
            use_bias = norm_layer == nn.InstanceNorm2d
        downconv = nn.Conv2d(in_size, out_size, kernel_size=4,
                             stride=2, padding=1, bias=use_bias)
        downrelu = nn.LeakyReLU(0.2, True)
        downnorm = norm_layer(out_size)
...
class UNetUp(nn.Module):
    def __init__(self, in_size, out_size, norm_layer, is_output_layer=False, use_dropout=False):
        ...
        if type(norm_layer) == functools.partial:
            ...
            use_bias = norm_layer == nn.InstanceNorm2d
        upconv = nn.ConvTranspose2d(in_size * 2, out_size,
                                    kernel_size=4, stride=2,
                                    padding=1, bias=use_bias)
        uprelu = nn.ReLU(True)
        model = [uprelu, upconv]
        if not is_output_layer:
            upnorm = norm_layer(out_size)
            model += [upnorm]
        if use_dropout:
    ...
    def forward(self, x, skip_input):
        ...
        x = self.model(torch.cat((x, skip_input), 1))
        return x
